 ...
 **/
  window.onload = function () {
    ...
    if (config.VocalizeNdiapo.val === 1)
      $(config.SlideCounter.id).setAttribute("aria-live", "polite");
    if (config.VocalizeNdiapo.val === 1)
      $(config.SlideCounter.id).setAttribute("aria-atomic", "false");
    if (config.GotoBnext.val === 1) $(config.Bnext.id).focus();
    if (config.SoundTxt.val === 1) SetSon(config.Stexte.file, config.Stexte.id);
    if (config.SoundSlide.val === 1)
      SetSon(config.Sdiapo.file, config.Sdiapo.id);
    if (config.SoundSlide1.val === 1)
      SetSon(config.Sdiapo1.file, config.Sdiapo1.id);
    if (config.SoundSlideEnd.val === 1)
      SetSon(config.Sdiapoend.file, config.Sdiapoend.id);
    //font dys
    SetFontFamily();
    //contrast
    //SetContrast();
    ...
    document.addEventListener(
      ...
      function (e) {
        ...
        if (config.ModePlan.val === 0) {
          if (
            key(e) === 32 &&
            !document.getElementById(config.Setting.id).contains(e.target)
          ) {
            GotoNext();
            e.preventDefault();
          }
          if (key(e) === 39 || key(e) === 34) {
            GotoNext();
            e.preventDefault();
          }
          if (key(e) === 37 || key(e) === 33 || key(e) == 132) {
            GotoPrev();
            e.preventDefault();
          }
          if (key(e) === 36) {
    ...
    $(config.Select.id).addEventListener(
      "change",
      function () {
    ...
    $(config.Bselect.id).addEventListener(
      "blur",
      function () {
    ...
    $(config.Bsubmit.id).addEventListener(
      "click",
      function (event) {
        SetConfig();
    ...
    $(config.BfullScreen.id).addEventListener(
      "click",
      function () {
        screenfull.toggle();
      },
    ...
    PrefixedEvent($(config.Flap.id), "AnimationEnd", function () {
      $(config.Flap.id).removeAttribute("class");
    });
    ...
    $(config.Nav.id).addEventListener(
      "focus",
      function () {
...
**/
  ...
  function InitTabSlide() {
    ...
    if (config.SoundSlide1.val === 1 && current === 0)
      $(config.Sdiapo1.id).play();
  ...
  function SetTabTextSlide(Cprev) {
    ...
    if (TabTextSlide.length > 0) {
      ...
      for (var i = 0, len = TabTextSlide.length; i < len; i++) {
        Cprev === 1
          ? (TabTextSlide[i].style.visibility = "visible")
          : (TabTextSlide[i].style.visibility = "hidden");
      }
  ...
  function SetOwnerInfo() {
    ...
    $(config.OwnerInfo.id).setAttribute(
      "aria-labelledby",
      config.OwnerInfo.Ctitle
    );
  }
  function SetHelpInfo() {
    ...
    $(config.HelpInfo.id).setAttribute(
      "aria-labelledby",
      config.HelpInfo.Ctitle
    );
  }
  /* Open summary modal */
  function OpenToc() {
    ...
    if (config.SumModOn.val === 1) {
      ...
      $(config.Summary.id).setAttribute(
        "aria-labelledby",
        config.Summary.Ctitle
      );
    } else {
      if (config.ModePlan.val === 0) {
        ...
        $(config.Wrapper.id).setAttribute(
          "class",
          config.Wrapper.classSettingLeft
        );
      }
    }
    if (styleSummary.getPropertyValue("display") === "block") {
      ...
      if ($(config.Btoc.id).querySelector("img"))
        $(config.Btoc.id)
          .querySelector("img")
          .setAttribute("alt", lang.Btoc.altOpen);
      openDial = null;
      ...
      if ($(config.Btoc.id).querySelector("img"))
        $(config.Btoc.id)
          .querySelector("img")
          .setAttribute("alt", lang.Btoc.altClose);
  ...
  function OpenConfig() {
    if ($(config.Setting.id).style.display === "block") {
      ...
      if ($(config.Bconfig.id).querySelector("img"))
        $(config.Bconfig.id)
          .querySelector("img")
          .setAttribute("alt", lang.Bconfig.altOpen);
      openDial = null;
      ...
      if ($(config.Bconfig.id).querySelector("img"))
        $(config.Bconfig.id)
          .querySelector("img")
          .setAttribute("alt", lang.Bconfig.altClose);
  ...
  function OpenOwnerInfo() {
    ...
    if (styleInfo.getPropertyValue("display") === "none") {
      ...
      if ($(config.BownerInfo.id).querySelector("img"))
        $(config.BownerInfo.id)
          .querySelector("img")
          .setAttribute("alt", lang.BownerInfo.altClose);
      ...
      if ($(config.BownerInfo.id).querySelector("img"))
        $(config.BownerInfo.id)
          .querySelector("img")
          .setAttribute("alt", lang.BownerInfo.altOpen);
      openDial = null;
    }
  }
  /* Open help info */
  function OpenHelpInfo() {
    ...
    if (styleHelp.getPropertyValue("display") === "none") {
      ...
      if ($(config.BhelpInfo.id).querySelector("img"))
        $(config.BhelpInfo.id)
          .querySelector("img")
          .setAttribute("alt", lang.BhelpInfo.altClose);
      ...
      if ($(config.BhelpInfo.id).querySelector("img"))
        $(config.BhelpInfo.id)
          .querySelector("img")
          .setAttribute("alt", lang.BhelpInfo.altOpen);
      OpenDial = null;
    }
  }
  /* Close modal */
  function CloseToc() {
    switch (openDial.getAttribute("id")) {
      ...
      case config.OwnerInfo.id:
        ...
        $(config.Nav.id).classList.remove("navbar-invisible");
        $(config.Nav.id).classList.add("navbar-visible");
        $(config.BownerInfo.id).setAttribute(
          "title",
          lang.BownerInfo.titleOpen
        );
        if ($(config.BownerInfo.id).querySelector("img"))
          $(config.BownerInfo.id)
            .querySelector("img")
    ...
    if (config.ModePlan.val === 1)
      $(config.Wrapper.id).removeAttribute("class");
    openDial = null;
  }
  ...
  function SetActiveSummaryLink() {
    ...
    for (var i = 0, len = summaryList.length; i < len; i++) {
      ...
      if (hashRef == linkRef[1]) {
        ...
        summaryList[i].setAttribute(
          "title",
          linkText + lang.SummaryLinkActiveTitle
        );
  ...
  function breadcrumb() {
    ...
    if (!headerH2) {
      if (current < TabSlide.length - 1) {
        ...
        var ndxHeader = parseInt(
          TabSlide[current].getAttribute("data-breadcrumb")
        );
        if (ndxHeader) {
          ...
          var target = document
            .getElementById(config.Breadcrumb.id)
            .querySelector("ul ");
  ...
  function ObjectFullScreenSetting() {
    var objTarget = document.querySelectorAll(
      "." + config.ObjectFullScreen.classSetting
    );
    for (var i = 0, len = objTarget.length; i < len; i++) {
      objTarget[i].setAttribute("tabindex", "0");
      objTarget[i].addEventListener(
        "click",
        function () {
  ...
  function DownloadDoc() {
    if ($(config.Bdownload.idFile).getAttribute("data-url")) {
      ...
      var urlDoc =
        "download/" +
        $(config.Bdownload.idFile).getAttribute("data-filename") +
        "." +
        $(config.Bdownload.idFile).getAttribute("data-type");
    }
    window.open(urlDoc);
  }
  ...
  function toggleFullScreenIcon(fullscreen) {
    if (fullscreen) {
      $(config.BfullScreen.id).setAttribute(
        "title",
        lang.BfullScreen.titleExit
      );
 ...
 **/
  ...
  function SetAdaptive() {
    ...
    if (config.SoundSlide1.val === 1 && current === 0)
      $(config.Sdiapo1.id).play();
    if (config.SoundSlideEnd.val === 1 && current === TabSlide.length - 1)
      $(config.Sdiapoend.id).play();
    if (config.UpWindowTitle.val === 1) MajTitreW();
  }
  ...
  function GetTitre() {
    ...
    if (target[1].tagName === "H2") {
      ...
      var Txt = document.createTextNode(
        TabSlide[current].getAttribute("aria-label")
      );
  ...
  function SetFontFamily() {
    if (config.FontDyslexia.val === 1) {
      document
        .querySelector("body")
        .classList.add(config.FontDyslexia.classSettingEffect);
    } else {
      document
        .querySelector("body")
        .classList.remove(config.FontDyslexia.classSettingEffect);
    }
    //linespace dys
    if (config.LineSpaceDyslexia.val === 1) {
      document
        .querySelector("body")
        .classList.add(config.LineSpaceDyslexia.classSettingEffect);
    } else {
      document
        .querySelector("body")
        .classList.remove(config.LineSpaceDyslexia.classSettingEffect);
    }
    //justify dys
    if (config.NoJustify.val === 1) {
      document
        .querySelector("body")
        .classList.add(config.NoJustify.classSettingEffect);
    } else {
      document
        .querySelector("body")
        .classList.remove(config.NoJustify.classSettingEffect);
    }
  }
...
**/
  ...
  function SetConfig() {
    var TabConfigRef = document.querySelectorAll(
  ...
  function ResetConfig() {
    ...
    for (var i = 0, len = TabConfigRef.length; i < len; i++) {
      ...
      if (TabConfigRef[i].nodeName === "INPUT") {
        if (config[ConfigRef].val === 1)
          TabConfigRef[i].setAttribute("checked", "checked");
  ...
  function GetConfig() {
    ...
    Url.indexOf("#") > 0
      ? (BaseUrl = BaseUrl = Url.slice(0, Url.indexOf("#")))
      : (BaseUrl = Url);
  ...
  function PushNavBar() {
    ...
    if ($(config.Bdownload.idFile)) {
      ...
      var DocumentMetadata =
        DataFile.getAttribute("data-name") +
        " (" +
        DataFile.getAttribute("data-type") +
        ", " +
        DataFile.getAttribute("data-weight") +
        " " +
        DataFile.getAttribute("data-lang") +
        " - " +
        lang.Bdownload.newWindow +
        ")";
      Cbutton.setAttribute("type", "button");
      Cbutton.setAttribute("id", config.Bdownload.id);
      Cbutton.setAttribute(
        "title",
        lang.Bdownload.title + " " + DocumentMetadata
      );
      Cbutton.className = config.BhelpInfo.classSetting;
      //Icon implementation
      var CspanFallback = document.createElement("SPAN");
      ...
      CspanText.setAttribute(
        "alt",
        lang.Bdownload.alt + " " + DocumentMetadata
      );
      CspanText.className = config.BhelpInfo.fallbackClass;
      CspanFallback.appendChild(CspanIcon);
      CspanFallback.appendChild(CspanText);
    ...
    $(config.WrapperNav.id).appendChild(CliIndex);
  }
...
**/
  ...
  function $(e, ndx) {
    ...
    if (document.getElementsByClassName(e)[x])
      elm = document.getElementsByClassName(e)[x];
    if (document.getElementsByTagName(e)[x])
      elm = document.getElementsByTagName(e)[x];
    return elm;
  }
  /* Get KeyCode */
